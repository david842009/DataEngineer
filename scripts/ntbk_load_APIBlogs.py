...
#              el directorio /opt/workspace/parquet/brz_stage/tbl_brz_Blogs                                                  #
...
process_name = "Extract_API_Blogs"
...
def read_table_data():
    path = "/opt/workspace/parquet/brz_stage/tbl_brz_blogs"
...
#==================================================================================================================================#
##Extracción datos API Blogs
API_URL = "https://api.spaceflightnewsapi.net/v4/blogs"
dataBlogs = fetch_paginated_data(API_URL, updated_at_gt=read_table_data())

# Procesar y guardar datos Blogs
df_blogs = spark.createDataFrame(dataBlogs, schema=blog_schema)
df_blogs = df_blogs.withColumn("published_at", to_timestamp("published_at", "yyyy-MM-dd'T'HH:mm:ssX"))
df_blogs = df_blogs.withColumn("updated_at", to_timestamp("updated_at", "yyyy-MM-dd'T'HH:mm:ss.SSSSSSX"))
df_blogs = df_blogs.withColumn("fecha_registro_dl", current_timestamp())
df_blogs = df_blogs.withColumn("estado_registro_dl", lit(1))

# Explodemos los arrays de estructuras
df_blogs = df_blogs.withColumn("authors_exploded", explode_outer("authors")) \
...
log_data = log_data.union(log_execution("Transformacion de los datos", process_name, "Completed", df_blogs.count()))
#==================================================================================================================================#
# Stage 5: Almacenamiento de la información                                                                                        #
#==================================================================================================================================#
path = "/opt/workspace/parquet/brz_stage/tbl_brz_blogs"

# Si el directorio no existe o está vacío, simplemente guardamos df_blogs
if not os.path.exists(path) or not os.listdir(path):
    df_blogs.write.mode("overwrite").parquet(path)
    log_data = log_data.union(log_execution("Union de datos", process_name, "Completed", df_blogs.count()))
    ...
    # Marcar como "inactivo" los registros en df_stage si su id existe en df_blogs usando JOIN
    df_stage = df_stage.alias("stage").join(
        df_blogs.select("id").alias("blogs"),
        on="id",
        how="left"
    ).withColumn(
        "estado_registro_dl",
        when(col("blogs.id").isNotNull(), lit(False))  # Si existe en df_blogs, poner False
    ...
    df_blogs_new = df_blogs.alias("new").join(
    ...
    # Unir datos
    df_final = df_stage.unionByName(df_blogs_new, allowMissingColumns=True)
