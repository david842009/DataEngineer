...
df_articles = spark.read.parquet("/opt/workspace/parquet/brz_stage/tbl_brz_articles")

# Limpiar texto y extraer palabras clave
df_text = df_articles.select(
    col("id"),
    col("title"),
    col("summary"),
    col("news_site"),
...
df_articles = df_articles.withColumn("domain_url", regexp_extract(col("url"), domain_pattern, 1))

df_sources = df_articles.groupBy("news_site", "domain_url").agg(count("id").alias("count")) \
...
df_trends = df_trends.withColumn("source", lit("articles"))
df_trends.write.mode("overwrite").parquet("/opt/workspace/parquet/slv_stage/tbl_slv_trends")
df_sources.write.mode("overwrite").parquet("/opt/workspace/parquet/slv_stage/tbl_slv_sources")
