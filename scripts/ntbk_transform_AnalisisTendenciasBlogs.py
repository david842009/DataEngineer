...
df_blogs = spark.read.parquet("/opt/workspace/parquet/brz_stage/tbl_brz_blogs")

# Limpiar texto y extraer palabras clave
df_text = df_blogs.select(
    col("id"),
    col("title"),
    col("summary"),
    col("news_site"),
...
df_blogs = df_blogs.withColumn("domain_url", regexp_extract(col("url"), domain_pattern, 1))

df_sources = df_blogs.groupBy("news_site", "domain_url").agg(count("id").alias("count")) \
...
df_trends = df_trends.withColumn("source", lit("blogs"))
