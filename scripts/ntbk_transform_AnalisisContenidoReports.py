...
# Inicializar Spark
spark = SparkSession.builder.appName("reportsContentAnalysis").getOrCreate()

# Cargar datos desde Parquet
df = spark.read.parquet("/opt/workspace/parquet/brz_stage/tbl_brz_reports").select("id", "title", "summary")
...
df_final = df_final.withColumn("source", lit("reports"))
