...
# Inicializar Spark
spark = SparkSession.builder.appName("ArticleContentAnalysis").getOrCreate()

# Cargar datos desde Parquet
df = spark.read.parquet("/opt/workspace/parquet/brz_stage/tbl_brz_articles").select("id", "title", "summary","news_site")
...
df_entities = df_entities.select("id", "title", "summary", "clean_summary", "news_site",
...
df_final = df_final.withColumn("source", lit("articles"))
df_final.write.mode("overwrite").parquet("/opt/workspace/parquet/slv_stage/tbl_slv_analysis")
